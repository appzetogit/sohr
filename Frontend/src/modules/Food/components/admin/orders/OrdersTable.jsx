...
import { Eye, Printer, ArrowUpDown, Loader2, Check, X, Trash2, RefreshCw, Volume2, PackageCheck, Bike } from "lucide-react"
...
export default function OrdersTable({
  ...
  // Hand-picking a rider is available right up until one has accepted: before that
  // the order is either waiting for the admin or holding an offer nobody took, and
  // reassigning simply replaces that offer.
  const canAssignDeliveryPartner = (order) => {
    const backendStatus = String(order?.status || "").trim().toLowerCase()
    return (
      ["created", "confirmed", "preparing", "ready_for_pickup", "ready", "reached_pickup"].includes(
        backendStatus,
      ) &&
      order?.dispatch?.status !== "accepted" &&
      !order?.deliveryState?.pickedUpAt
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      {onAssignDeliveryPartner ? (
                        <button
                          onClick={() =>
                            canAssignDeliveryPartner(order) && onAssignDeliveryPartner(order)
                          }
                          disabled={
                            actionLoadingOrderId === (order.id || order.orderId) ||
                            !canAssignDeliveryPartner(order)
                          }
                          className={`inline-flex items-center justify-center gap-1 rounded px-3 py-1.5 text-xs font-medium transition-colors ${canAssignDeliveryPartner(order)
                              ? "bg-emerald-600 text-white hover:bg-emerald-700"
                              : "bg-slate-100 text-slate-400 cursor-not-allowed"
                            } disabled:cursor-not-allowed disabled:opacity-60`}
                          title={
                            canAssignDeliveryPartner(order)
                              ? "Fill in the order details and send it to one delivery partner"
                              : "Available until a delivery partner has accepted the order"
                          }
                        >
                          {actionLoadingOrderId === (order.id || order.orderId) ? (
