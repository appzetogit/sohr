...
import AssignDeliveryModal from "@food/components/admin/orders/AssignDeliveryModal"
import ManualOrderModal from "@food/components/admin/orders/ManualOrderModal"
...
export default function OrdersPage({ statusKey = "all" }) {
  ...
  const [orderForAssign, setOrderForAssign] = useState(null)
  ...
  useEffect(() => {
    ...
    // A hand-assigned order the rider rejected or ignored comes back here, not to the
    // broadcast hunt — so the admin has to be told to pick someone else.
    const handleOrderUnassigned = (payload = {}) => {
      toast.warning(
        payload?.reason === "timeout"
          ? `Order ${payload?.orderId || ""} was not accepted in time`
          : `Order ${payload?.orderId || ""} was rejected by the delivery partner`,
        { description: "Assign another delivery partner." },
      )
      fetchOrdersRef.current({ silent: true, withRingCheck: false, force: true })
    }
    ...
    socket.on("admin_order_unassigned", handleOrderUnassigned)
    ...
    return () => {
      ...
      socket.off("admin_order_unassigned", handleOrderUnassigned)
      socket.disconnect()
  ...
  const handleAssignSubmit = async (payload) => {
    const orderIdToUse = orderForAssign?.id || orderForAssign?._id || orderForAssign?.orderId
    if (!orderIdToUse) {
      toast.error("Order ID not found")
      return
    }
    ...
    try {
      setIsAssigning(true)
      const response = await adminAPI.assignDeliveryPartner(orderIdToUse, payload)
      toast.success(
        response?.data?.message || "Order sent to the delivery partner",
      )
      setAssignModalOpen(false)
      setOrderForAssign(null)
      await fetchOrdersRef.current({ silent: true, withRingCheck: false, force: true })
    } catch (error) {
      debugError("Error assigning delivery partner:", error)
      toast.error(
  ...
  const handleCreateManualOrder = async (body) => {
    try {
      setIsCreatingManualOrder(true)
      const response = await adminAPI.createManualOrder(body)
      toast.success(response?.data?.message || "Order created")
      setManualOrderOpen(false)
      await fetchOrdersRef.current({ silent: true, withRingCheck: false, force: true })
    } catch (error) {
      debugError("Error creating manual order:", error)
  ...
  return (
    <div className="p-4 lg:p-6 bg-slate-50 min-h-screen w-full max-w-full overflow-x-hidden">
      ...
      <AssignDeliveryModal
        ...
        onOpenChange={(next) => {
          setAssignModalOpen(next)
          if (!next) setOrderForAssign(null)
        }}
      ...
      <OrdersTable 
        ...
        onAssignDeliveryPartner={handleOpenAssign}
        onCancelOrder={
          statusKey === "all" ||
          statusKey === "pending" ||
