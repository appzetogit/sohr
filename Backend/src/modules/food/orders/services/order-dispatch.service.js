...
import {
  ...
  haversineKm,
  isAdminAssignedOrder,
  notifyAdminOrderUnassigned,
  notifyOwnerSafely,
 ...
 */
export const DRIVER_ACCEPT_WINDOW_MS = 45000;
  ...
  const order = await FoodOrder.findOneAndUpdate(
    {
      ...
      // Hands off anything an admin hand-picked a rider for. Without this the stale
      // 'assigned' branch below reclaims a manual assignment ~50s later and broadcasts
      // it to every nearby rider — exactly what manual assignment exists to prevent.
      // $ne also matches orders written before this field existed.
      'dispatch.assignedBy': { $ne: 'ADMIN' },
      $or: [
...
export async function processDispatchTimeout(orderId, partnerId) {
  ...
  // Admin-assigned: an ignored offer returns to the admin to reassign by hand.
  // Never falls through to the broadcast hunt.
  if (isAdminAssignedOrder(order)) {
    ...
    logger.info(`Admin-assigned order ${orderId} not accepted in time — returned to admin.`);
    await notifyAdminOrderUnassigned(order, 'timeout');
    return;
  }
...
export async function resendDeliveryNotificationRestaurant(orderId, restaurantId) {
  ...
  order.dispatch.deliveryPartnerId = null;
  order.dispatch.offeredTo = [];
  // Explicitly asking for a resend is opting back into the broadcast hunt.
  order.dispatch.assignedBy = 'AUTO';
  await order.save();
