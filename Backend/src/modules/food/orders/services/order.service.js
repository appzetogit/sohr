...
/** Restaurant fields buildDeliverySocketPayload reads for the pickup half of the offer. */
const OFFER_RESTAURANT_FIELDS =
  'restaurantName location addressLine1 area city state phone ownerPhone coverImage coverImages galleryImages landmark zoneId';

/**
 * Order details the admin edits on the Assign dialog before the offer goes out.
 *
 * These all map onto fields the order already has — the rider payload is built from
 * them, so correcting an address or a phone here is what the rider actually receives.
 * Only keys the admin actually sent are touched; a blank field must not wipe what the
 * order already knows.
 */
function applyAdminOrderDetails(order, details) {
  if (!details || typeof details !== 'object') return;

  const set = (value) => value !== undefined && value !== null && String(value).trim() !== '';

  if (set(details.customerName)) order.customerName = String(details.customerName).trim();
  if (set(details.customerPhone)) order.customerPhone = String(details.customerPhone).trim();
  if (details.note !== undefined) order.note = String(details.note || '');
  if (details.deliveryInstructions !== undefined) {
    order.deliveryInstructions = String(details.deliveryInstructions || '');
  }

  const addressPatch = details.address || {};
  if (Object.keys(addressPatch).length > 0) {
    const current = order.deliveryAddress?.toObject
      ? order.deliveryAddress.toObject()
      : { ...(order.deliveryAddress || {}) };
    const merged = { ...current };
    for (const [key, value] of Object.entries(addressPatch)) {
      if (value !== undefined && value !== null && String(value).trim() !== '') {
        merged[key] = value;
      }
    }
    if (set(details.customerName)) {
      merged.name = String(details.customerName).trim();
      merged.fullName = String(details.customerName).trim();
    }
    if (set(details.customerPhone)) merged.phone = String(details.customerPhone).trim();
    order.deliveryAddress = normalizeDeliveryAddress(merged);
  }

  const amount = Number(details.amount);
  if (Number.isFinite(amount) && amount >= 0) {
    order.pricing = {
      ...(order.pricing?.toObject ? order.pricing.toObject() : order.pricing || {}),
      subtotal: amount,
      total: amount,
    };
    if (order.payment) order.payment.amountDue = amount;
  }

  if (set(details.paymentMethod)) {
    const method = String(details.paymentMethod).trim();
    order.payment = {
      ...(order.payment?.toObject ? order.payment.toObject() : order.payment || {}),
      method,
      status: method === 'cash' ? 'cod_pending' : order.payment?.status || 'paid',
    };
  }

  const riderEarning = Number(details.riderEarning);
...
/**
 * Hand one specific rider the order.
 *
 * Same two channels the broadcast uses (socket for a foregrounded app, FCM for a
 * locked or killed one) but addressed to a single room. The rider's accept endpoint
 * already recognises a direct assignment, so nothing on their side changes.
 */
async function sendOrderOfferToPartner(order, deliveryPartnerId) {
  const restaurant = await FoodRestaurant.findById(order.restaurantId)
    .select(OFFER_RESTAURANT_FIELDS)
    .lean();
  const payload = buildDeliverySocketPayload(order, restaurant);
  const acceptanceDeadlineAt = new Date(
    Date.now() + dispatchService.DRIVER_ACCEPT_WINDOW_MS,
  );

  try {
    const io = getIO();
    if (io) {
      io.to(rooms.delivery(deliveryPartnerId)).emit('new_order', {
        ...payload,
        acceptanceDeadlineAt,
    ...
    logger.warn(`Admin assign socket emit failed: ${err?.message || err}`);
  }
  ...
  try {
    await notifyOwnersActionableAlert(
      [{ ownerType: 'DELIVERY_PARTNER', ownerId: deliveryPartnerId }],
      {
        ...
        body: `Order #${order.order_id || order._id} has been assigned to you. Tap to accept.`,
        // Tag and channel id are the contract with the rider app's fcm_service —
        // change one and you must change the other.
        androidTag: `order_${order._id.toString()}`,
        androidChannelId: 'incoming_orders_channel_v3',
        data: dispatchService.buildIncomingOrderPushData(order, payload, acceptanceDeadlineAt),
      },
    );
  } catch (err) {
    logger.warn(`Admin assign push failed for order ${order._id}: ${err?.message || err}`);
  }

  // Ignored offers must come back to the admin rather than sit assigned forever.
  // processDispatchTimeout returns admin-assigned orders to the admin, never to the
  // broadcast hunt.
  void addOrderJob(
    {
    ...
    { delay: dispatchService.DRIVER_ACCEPT_WINDOW_MS },
  ).catch((err) => {
    logger.warn(`Admin assign timeout check enqueue failed: ${err?.message || err}`);
  });
}

export async function assignDeliveryPartnerAdmin(orderId, dto, adminId) {
  const deliveryPartnerId = String(dto?.deliveryPartnerId || '').trim();
  const identity = buildOrderIdentityFilter(orderId);
  if (!identity) throw new ValidationError('Order id required');
  ...
  // A rider already on a trip cannot take a second one — their own accept endpoint
  // refuses it, so assigning would produce an offer that can never be accepted.
  const busyPartnerIds = await getBusyDeliveryPartnerIds();
  if (busyPartnerIds.has(String(deliveryPartnerId))) {
    throw new ValidationError('That delivery partner is already on an active delivery');
  }

  applyAdminOrderDetails(order, dto?.orderDetails);

  const now = new Date();
  order.dispatch.status = 'assigned';
  order.dispatch.assignedBy = 'ADMIN';
  ...
  order.dispatch.offeredTo.push({ partnerId: deliveryPartnerId, at: now, action: 'offered' });
  ...
  pushStatusHistory(order, {
    ...
    from: order.orderStatus,
    to: order.orderStatus,
    note: `Delivery partner assigned by admin${partner.name ? `: ${partner.name}` : ''}`,
  });
  await order.save();

  await sendOrderOfferToPartner(order, deliveryPartnerId);

  return normalizeOrderForClient(order);
}

/**
 * Order taken over WhatsApp and typed in by the admin.
 *
 * No cart, no menu lookup, no pricing engine: the admin already agreed a total with
 * the customer. It still has to be a real FoodOrder so every downstream path (rider
 * offer, tracking, earnings, cash-in-hand, reports) keeps working unchanged, which
 * means satisfying the schema's required userId/restaurantId:
 *  - userId  — findOrCreateUserByPhone on the customer's WhatsApp number, the same
 *              helper OTP login uses. Beats loosening the schema, which every
 *              populate/room/aggregation in the codebase assumes is set.
 *  - restaurantId — the admin picks the pickup point; the rider offer is built from
 *              its name, address and coordinates.
 *
 * Created straight into 'confirmed' (there is no restaurant app in this flow) and
 * marked assignedBy ADMIN so auto-dispatch never broadcasts it — it waits for the
 * admin to pick a rider.
 */
export async function createManualOrderAdmin(dto, adminId) {
  const restaurantId = toObjectId(dto.restaurantId, 'Restaurant ID');
  ...
  const user = await findOrCreateUserByPhone({ phone: customerPhone });

  const deliveryAddress = normalizeDeliveryAddress({
    label: 'Home',
    name: customerName,
    fullName: customerName,
    street: String(dto.address.street).trim(),
    additionalDetails: String(dto.address.additionalDetails || '').trim(),
    city: String(dto.address.city).trim(),
    state: String(dto.address.state).trim(),
    zipCode: String(dto.address.zipCode || '').trim(),
    phone: customerPhone,
  ...
  // Distance is best-effort: it only exists when the admin pinned coordinates, and a
  // missing one must not block the order — it costs the rider the km readout and the
  // distance-based earning, nothing more.
  let distanceKm = null;
  try {
    distanceKm = await getDeliveryDistanceKm(restaurant, deliveryAddress);
  } catch (err) {
    logger.warn(`Manual order distance lookup failed: ${err?.message || err}`);
  }
  if (Number.isFinite(Number(distanceKm))) {
    distanceKm = Number(Number(distanceKm).toFixed(2));
    pricing.distanceKm = distanceKm;
    pricing.roadDistanceKm = distanceKm;
  } else {
    distanceKm = null;
  }

  let riderEarning = Number(dto.riderEarning);
  if (!Number.isFinite(riderEarning) || riderEarning < 0) {
    try {
      riderEarning = Number(calculateRiderEarning(await loadActiveFeeSettings(), distanceKm)) || 0;
    } catch (err) {
      logger.warn(`Manual order rider earning calculation failed: ${err?.message || err}`);
      riderEarning = 0;
    }
  }
  ...
  const order = new FoodOrder({
    ...
    // One line item: a WhatsApp order is agreed as a description and a total, not an
    // itemised cart. Everything downstream only needs items to be non-empty.
    items: [
      {
        itemId: 'manual',
        name: String(dto.items).trim(),
        price: amount,
        quantity: 1,
      },
    ],
    ...
    dispatch: { status: 'unassigned', assignedBy: 'ADMIN' },
    statusHistory: [
      {
        at: new Date(),
        byRole: 'ADMIN',
        ...
        from: '',
        to: 'confirmed',
        note: 'Order received over WhatsApp and created by admin',
      },
    ],
    note: String(dto.note || ''),
    deliveryInstructions: String(dto.deliveryInstructions || ''),
  ...
  await order.save();
  logger.info(`Manual order ${order.order_id || order._id} created by admin ${adminId}`);
