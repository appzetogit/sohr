...
export async function rejectOrderDelivery(orderId, deliveryPartnerId) {
  ...
  // An order the admin hand-assigned goes back to the admin, not to the broadcast
  // hunt — otherwise the rejected order lands on whichever rider is nearest, which is
  // the exact outcome hand-picking exists to avoid.
  if (isAdminAssignedOrder(order)) {
    void notifyAdminOrderUnassigned(order, 'rejected').catch((error) =>
      logger.warn(`Admin unassign notification failed: ${error.message}`),
    );
  } else {
