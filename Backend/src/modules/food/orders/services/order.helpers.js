...
/** True when an admin hand-picked the rider, so auto-dispatch must stay out of it. */
export function isAdminAssignedOrder(orderLike) {
  return String(orderLike?.dispatch?.assignedBy || '') === 'ADMIN';
}

/**
 * A manually assigned order the rider rejected or ignored goes back to the admin,
 * never to the broadcast hunt. Push, because the admin is not necessarily looking
 * at the orders table when a rider declines.
 */
export async function notifyAdminOrderUnassigned(orderLike, reason) {
  const orderMongoId = String(orderLike?._id || '');
  const displayId = orderLike?.order_id || orderMongoId;
  const payload = {
    orderMongoId,
    orderId: displayId,
    reason,
  ...
  try {
    const io = getIO();
    if (io) io.to(rooms.admin()).emit('admin_order_unassigned', payload);
  } catch (err) {
    logger.warn(`admin_order_unassigned emit failed: ${err?.message || err}`);
  }
  ...
  await notifyOwnersSafely([{ ownerType: 'ADMIN', ownerId: 'GLOBAL' }], {
    ...
    body:
      reason === 'timeout'
        ? `Order #${displayId} was not accepted in time. Assign another delivery partner.`
        : `Order #${displayId} was rejected by the delivery partner. Assign another one.`,
    data: { type: 'admin_order_unassigned', ...payload },
  });
}
